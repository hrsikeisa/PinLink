...
const SharePinLinkModal = ({ modalOpen, setModalOpen, username }: GetStartedModalProps) => {
  const urls = ['pinlink.vercel.app']
  const socials = [
    {
      icon: FaTwitter,
      name: 'twitter',
    ...
    },
    {
      icon: FaWhatsapp,
      name: 'whatsapp',
      link: `https://wa.me/?text=Check%20out%20my%20PinLink%20over%20on%20${'pinlink.vercel.app'}/${username}`,
    },
    {
      icon: FaLinkedin,
      name: 'linkedin',
      link: `https://www.linkedin.com/sharing/share-offsite/?url=${'pinlink.vercel.app'}/${username}`,
    },
    {
      icon: FaEnvelope,
      name: 'email',
      link: `mailto:?subject=check%20out%20my%20brand%20new%20PinLink%20%3A%29&body=Check%20it%20out%20over%20on%20${'pinlink.vercel.app'}/${username}`,
    },
  ]
        ...
        <ModalBody pb={20} px={{ base: 4, md: 20 }}>
          <VStack pt={12} spacing={8}>
            <Heading fontSize={{ base: 'md', md: 'lg' }}>
              Share your PinLink with the world 🌎❤️
            </Heading>
            <SimpleGrid columns={1} textAlign="center" spacing={4}>
              {urls.map((url, index) => (
                <HStack
...
export default SharePinLinkModal

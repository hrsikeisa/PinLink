...
async function middleware(request: NextRequest) {
  ...
  if (!hostname) return NextResponse.redirect('https://pinlinklink.com')

  // get our base url [https://pinlinklink.com]
  const BASE_URL = getBaseURL(hostname)

  // if the domain is a customdomain, and it's root path, redirect to pinlinklink.com
  if (CUSTOM_DOMAINS.includes(hostname) && pathname === '/') {
    return NextResponse.redirect('https://pinlinklink.com')
  }
  ...
  if (!userId) return NextResponse.redirect('https://pinlinklink.com')
